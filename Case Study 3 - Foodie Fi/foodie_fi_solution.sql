...
select 
    monthname(start_date) as month_name , count(*) as trials
from subscriptions 
where plan_id = 0
...
select 
    ...
    concat(round(sum(case when plan_name = 'churn' then 1 else 0 end )* 100.0/count(distinct customer_id),1), "%") as percent
from subscriptions s 
...
with cte as 
(
    select *, lag(plan_name, 1) over(partition by customer_id order by start_date) as lg
    from subscriptions s inner join
    plans p using(plan_id)
    )
select 
    count(*) as customer_churn, 
    (select count(distinct customer_id) from subscriptions) as total_customers,
    concat(round(count(*) * 100.0/(select count(distinct customer_id) from subscriptions), 0), "%") as percent
from cte 
where plan_name = 'churn' and lg = 'trial';
...
select 
    plan_name, count(*) as customer_count, 
    concat(round(count(*)* 100.0/(select count(distinct customer_id) from subscriptions),2), "%") as total_customers 
from cte 
where rn = 2
group by plan_name;
...
select 
    ...
    concat(round(count(plan_name) / max(total_customers) * 100, 1), ' %') as percent
from cte, cte2
where rk = 1
group by plan_name
order by count desc;
...
with cte as
(
    select * ,lag(start_date, 1) over(partition by customer_id order by start_date) as lg 
    from subscriptions s inner join plans p using(plan_id)
    where plan_name in ('trial', 'pro annual')
    )
select 
    round(avg(datediff(start_date, lg)),0) as avg_turnaround
from cte 
...
(
    select 0 as a, 30 as b
    union all
    select b+1 , b+30 from cte where b < 346
    )
, cte2 as
(
    select a, b, concat(a, '-', b) as period from cte
    )
, cte3 as 
(
    select * , lag(start_date, 1) over(partition by customer_id order by start_date) as lg 
...
, cte4 as 
(
    select datediff(start_date, lg) as date_diff from cte3 where lg is not null
    )
select  
    period, 
    count(date_diff) as count_diff
from cte4 
...
select 
    case when count(customer_id) is null then 0 else count(customer_id) end as downgraded_customers
from cte 
